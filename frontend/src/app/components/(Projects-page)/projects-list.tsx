...
export default function ProjectsList() {
  ...
  const queryFilters = selectedCategory
    ? `filters[categories][name][$eq]=${selectedCategory}`
    : "";
  ...
  } = useStrapiData("categories", locale);

  const t = useTranslations("Projects");

  if (projectsLoading || categoriesLoading) return <p className="min-h-96"></p>;
  if (projectsError || categoriesError)
    return (
      <p className="pt-24 pb-12 container mx-auto flex justify-center">
        Error: {projectsError} {categoriesError}
      </p>
    );
